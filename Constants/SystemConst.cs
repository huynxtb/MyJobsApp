namespace MyJobsApp.Constants
{
    public static class SystemConst
    {
        public static string BackupSrcFolder = "BackupFile\\";
        public static string MaterialStoreBackup = "MaterialStoreBackup";
    }
    
    public static class OwnerInfo
    {
        public static string Bank = "TP Bank";
        public static string BankNumber = "04178006901";
        public static string BankOwnerName = "NGUYEN XUAN HUY";
        public static string PhoneNumber = "0399059205";
        public static string EmailAddress = "huynx11.dev@gmail.com";
        public static string Website = "https://devswim.com";
        public static string Signature = "Devswim.com";
    }
}